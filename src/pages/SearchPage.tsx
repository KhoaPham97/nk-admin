      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <span className="text-lg dark:text-white">
            Kết quả tìm kiếm: <span className="font-bold">"{query}"</span>
          </span>
          {products.length > 0 && (
        ...
        {isLoading ? (
          ...
          <div>
            <p className="text-lg dark:text-white mb-4">
              Không tìm thấy sản phẩm nào, nhưng đây là các danh mục phù hợp:{" "}
            </p>
            <div className="grid xl:grid-cols-6 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-2">
              {categoryResults.map((category) => (
                <div
                  key={category.slug}
                  className="bg-gray-100 dark:bg-slate-600 dark:text-white px-4 py-4  cursor-pointer hover:bg-gray-200 dark:hover:bg-slate-500"
